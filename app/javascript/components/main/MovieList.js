    ...
    <div className="container mx-auto p-4 mt-8 bg-white shadow-lg rounded-lg border border-gray-200">
      ...
      {!showDetail && (
        ...
        >
          ...
          <button className="btn bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={applyFilters}>Apply Filters</button>
        </div>
      )}
          ...
          {!showDetail ? (
            ...
            <div className="grid grid-cols-1 gap-4">
              {movies.map((movie, index) => (
                <div key={`${movie.id}-${index}`} onClick={() => handleMovieSelect(movie)}>
                  <MovieCard movie={movie} />
                </div>
              ))}
            </div>
          ) : (
            // Renderiza los detalles de la película seleccionada
